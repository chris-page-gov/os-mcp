...
ROUTING_PROMPTS: Dict[str, str] = {
    "routing_network_build_and_summarise": (
        "Goal: Build and summarise a routing network for a bbox. Steps: 1) os_ngd_init_mapping_workflow(); 2) get_routing_data(bbox='<bbox>', build_network=True); "
    ...
    "routing_restriction_audit": (
        "Goal: Audit restrictions for a bbox. Steps: 1) os_ngd_init_mapping_workflow(); 2) get_routing_data(bbox='<bbox>', build_network=True); "
